 ...
 * 
 */
public class JPanelPuesto extends javax.swing.JPanel {
    ...
    private final short DEPARTAMENTO = 2;
    ...
    public JPanelPuesto(JFrame parent) {
        initComponents();
        this.auth = new Auth();
        this.parent = parent;
        if (auth.isNotAuthorized(PUESTOS_ELIMINAR)) {
            jButtonEliminar.setEnabled(false);
            jMenuItemEliminar.setEnabled(false);
        }
        if (auth.isNotAuthorized(PUESTO_CREAR)) {
            jButtonNuevo.setEnabled(false);
            jMenuItemNuevo.setEnabled(false);
        }
        if (auth.isNotAuthorized(PUESTOS_EDITAR)) {
    ...
    private void cargarDatos(short page) {
        try {
            ...
            PuestoPage response = puestoController.find(search, by, elements, page);
            if (response != null) {
                this.puestos = response.getData();
                if (!this.puestos.isEmpty()) {
                    if (auth.isAuthorized(PUESTOS_ELIMINAR)) {
                        jButtonEliminar.setEnabled(true);
                    }
                    if (auth.isAuthorized(PUESTOS_EDITAR)) {
    ...
    private void cargarTabla(){
        try {
            ...
            headerType[DEPARTAMENTO] = java.lang.String.class;
            ...
            for (int i = 0; i < puestos.size(); i++) {
                ...
                defaultTableModel.setValueAt(puesto.getDescripcion(),  i, 1);
                defaultTableModel.setValueAt(puesto.getDepartamento().getNombre(),  i, 2);
            }
    ...
    private void formatearTabla(){
        try {
            ...
            jTable.getColumnModel().getColumn(NOMBRE).setMaxWidth(450);
            jTable.getColumnModel().getColumn(DESCRIPCION).setMinWidth(50);
            jTable.getColumnModel().getColumn(DEPARTAMENTO).setMinWidth(250);
            jTable.getColumnModel().getColumn(DEPARTAMENTO).setMaxWidth(450);
    ...
    private void ver(){
        ...
        if(selected >= 0 && selected < puestos.size()){
            Puesto puesto = puestos.get(selected);
            JDialogPuestoVer jddv = new JDialogPuestoVer(parent, puesto);
        }else{
    ...
    private void editar(){
        ...
        if(selected >= 0 && selected < puestos.size()){
            Puesto puesto = puestos.get(selected);
            JDialogPuestoEditar jdpe = new JDialogPuestoEditar(parent, puesto);
            init();
        }else{
            JOptionPane.showMessageDialog(parent, "No se puede realizar la accion", "Warning", JOptionPane.WARNING_MESSAGE);
        }
    ...
    private void eliminar(){
        ...
        if(selected >= 0 && selected < puestos.size()){
            ...
            int op = JOptionPane.showConfirmDialog(parent, "¿Estás seguro de eliminar el puesto de "+puesto.getNombre()+"?", "Warning", JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE);
            if(op == JOptionPane.YES_OPTION){
                try {
                    PuestoController pc = new PuestoController();
                    pc.delete(puesto.getClave());
                    init();
                } catch (Exception ex) {
                    Logger.getLogger(JPanelPuesto.class.getName()).log(Level.SEVERE, null, ex);
    ...
    private void initComponents() {
        ...
        jComboBoxBy.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "Nombre", "Descripción", "Departamento" }));
        ...
        jPanel2Layout.setHorizontalGroup(
            ...
            .addGroup(javax.swing.GroupLayout.Alignment.TRAILING, jPanel2Layout.createSequentialGroup()
                ...
                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                .addComponent(jTextFieldSearch, javax.swing.GroupLayout.DEFAULT_SIZE, 120, Short.MAX_VALUE)
                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
    ...
    private void jButtonNuevoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonNuevoActionPerformed
        JDialogPuestoNuevo jdpn = new JDialogPuestoNuevo(parent);
        init();
