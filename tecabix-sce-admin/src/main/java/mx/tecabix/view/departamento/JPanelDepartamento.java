...
public class JPanelDepartamento extends javax.swing.JPanel {
    ...
    public JPanelDepartamento(JFrame parent) {
        initComponents();
        this.auth = new Auth();
        this.parent = parent;
        if (auth.isNotAuthorized(DEPARTAMENTO_ELIMINAR)) {
            jButtonEliminar.setEnabled(false);
            jMenuItemEliminar.setEnabled(false);
        }
        if (auth.isNotAuthorized(DEPARTAMENTO_CREAR)) {
            jButtonNuevo.setEnabled(false);
            jMenuItemNuevo.setEnabled(false);
        }
        if (auth.isNotAuthorized(DEPARTAMENTO_EDITAR)) {
    ...
    private void cargarDatos(short page) {
        try {
            ...
            DepartamentoPage response = departamentoController.find(search, by, elements, page);
            if (response != null) {
                this.departamentos = response.getData();
                if (!this.departamentos.isEmpty()) {
                    if (auth.isAuthorized(DEPARTAMENTO_ELIMINAR)) {
                        jButtonEliminar.setEnabled(true);
                    }
                    if (auth.isAuthorized(DEPARTAMENTO_EDITAR)) {
    ...
    private void cargarTabla(){
        try {
            ...
            for (int i = 0; i < departamentos.size(); i++) {
                ...
                defaultTableModel.setValueAt(departamento.getNombre(),  i, 0);
                defaultTableModel.setValueAt(departamento.getDescripcion(),  i, 1);
            }
