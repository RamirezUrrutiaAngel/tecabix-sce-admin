...
public class JPanelPerfil extends javax.swing.JPanel {
    ...
    public JPanelPerfil(JFrame parent) {
        initComponents();
        this.auth = new Auth();
        this.parent = parent;
        if (auth.isNotAuthorized(PERFIL_ELIMINAR)) {
            jButtonEliminar.setEnabled(false);
            jMenuItemEliminar.setEnabled(false);
        }
        if (auth.isNotAuthorized(PERFIL_CREAR)) {
            jButtonNuevo.setEnabled(false);
            jMenuItemNuevo.setEnabled(false);
        }
        if (auth.isNotAuthorized(PERFIL_EDITAR)) {
    ...
    private void cargarDatos(short page) {
        try {
            ...
            PerfilPage response = perfilController.find(search, by, elements, page);
            if (response != null) {
                this.perfiles = response.getData();
                if (!this.perfiles.isEmpty()) {
                    if (auth.isAuthorized(PERFIL_ELIMINAR)) {
                        jButtonEliminar.setEnabled(true);
                    }
                    if (auth.isAuthorized(PERFIL_EDITAR)) {
    ...
    private void cargarTabla(){
        try {
            ...
            for (int i = 0; i < perfiles.size(); i++) {
                ...
                defaultTableModel.setValueAt(perfil.getNombre(),  i, 0);
                defaultTableModel.setValueAt(perfil.getDescripcion(),  i, 1);
            }
    ...
    private void ver(){
        ...
        if(selected >= 0 && selected < perfiles.size()){
            Perfil perfil = perfiles.get(selected);
            JDialogPerfilVer ver = new JDialogPerfilVer(parent, perfil);
        }else{
    ...
    private void editar(){
        ...
        if(selected >= 0 && selected < perfiles.size()){
            Perfil perfil = perfiles.get(selected);
            Perfil perfilActual = ConfiguracionConexion.getConfiguracionConexion().getSesion().getUsuario().getPerfil();
            if(perfil.getClave().equals(perfilActual.getClave())){
                JOptionPane.showMessageDialog(parent, "No puedes modificar el perfil en que te encuentras", "Warning", JOptionPane.WARNING_MESSAGE);
                return;
            }
            JDialogPerfilEditar dialogPerfilEditar = new JDialogPerfilEditar(parent, perfil);
            init();
        }else{
            JOptionPane.showMessageDialog(parent, "No se puede realizar la accion", "Warning", JOptionPane.WARNING_MESSAGE);
        }
    ...
    private void eliminar(){
        ...
        if(selected >= 0 && selected < perfiles.size()){
            ...
            Perfil perfilActual = ConfiguracionConexion.getConfiguracionConexion().getSesion().getUsuario().getPerfil();
            if(perfil.getClave().equals(perfilActual.getClave())){
                JOptionPane.showMessageDialog(parent, "No puedes eliminar el perfil en que te encuentras", "Warning", JOptionPane.WARNING_MESSAGE);
                return;
            }
            int op = JOptionPane.showConfirmDialog(parent, "¿Estás seguro de eliminar el perfil "+perfil.getNombre()+"?", "Warning", JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE);
            if(op == JOptionPane.YES_OPTION){
                try {
                    PerfilController perfilController = new PerfilController();
                    perfilController.delete(perfil.getClave());
                    init();
                } catch (Exception ex) {
